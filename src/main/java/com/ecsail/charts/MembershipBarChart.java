...
public class MembershipBarChart extends BarChart<String,Number> {
    ...
    private void addData() {
        for (StatsDTO s: stats) {
            switch (set) {
                case 1:
                    nonRenewData.add(new XYChart.Data<String,Number>(String.valueOf(s.getFiscalYear()),s.getNonRenewMemberships()));
                    break;
                case 2:
                    newMemberData.add(new XYChart.Data<String,Number>(String.valueOf(s.getFiscalYear()),s.getNewMemberships()));
                    break;
                case 3:
                    returnMemberData.add(new XYChart.Data<String,Number>(String.valueOf(s.getFiscalYear()),s.getReturnMemberships()));
            }
        }
    }
